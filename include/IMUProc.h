...
namespace imu_proc {
	...
	struct IMUData {
		...
		double cur_pan = 0.0;
		double cur_roll = 0.0;
		double dynamic_tilt_base = 0.0;
		double dynamic_pan_base = 0.0;
		double dynamic_roll_base = 0.0;
		...
		double acc_lon = 0.0;
		double acc_lat = 0.0;
		double acc_ground = 0.0;

		double cur_pan_rate = 0.0;
	};
	...
	public:
		...
		void setXBiasGyro(const double gyro_x_bias = 0.0);
		void setYBiasGyro(const double gyro_y_bias = 0.0);
		void setZBiasGyro(const double gyro_z_bias = 0.0);
		void configIMUCalib(std::string str_calib_file);
		void setAppMode(int app_mode) { this->app_mode_ = app_mode; };

		void getIMUData(imu_proc::IMUData &imu_data);
		bool getInitializeStatus() { return this->initialized_; }
		bool getAutoUpdateStatus() { return this->auto_update_; }
	...
	private:
		...
		//auto update
		bool auto_update_;		//true: automatically initialize init_angles when delta value achieves to a certain threshold
